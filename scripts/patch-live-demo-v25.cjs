...
const newUse = `function useAmbience(){
  ...
  const fade = async (gain, a,b,ms) => {
    const ctx = gain.context; const t0 = ctx.currentTime
    gain.gain.cancelScheduledValues(t0); gain.gain.setValueAtTime(a, t0)
    gain.gain.linearRampToValueAtTime(b, t0 + ms/1000)
    await new Promise(r => setTimeout(r, ms))
  }
  const start = async (type, vol) => {
    if (typeRef.current && typeRef.current === type && gainRef.current){ gainRef.current.gain.value = Math.max(0, Math.min(1, vol)); return }
    await stop()
  ...
  const setVolume = (vol) => { if (gainRef.current) gainRef.current.gain.value = Math.max(0, Math.min(1, vol)) }
...
fs.writeFileSync(path.join(ambDir,'README.txt'), 'Lege echte Natur-Sounds (WAV) hier ab: rain.wav, ocean.wav, wind.wav, forest.wav\n')

console.log('✅ Patch v25.1 angewendet (CJS minimal).')
